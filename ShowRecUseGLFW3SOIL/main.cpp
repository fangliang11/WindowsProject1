...
int main()
{
	...
	// Build and compile our shader program  编译和连接自定义着色器
	Shader ourShader("shader.vs", "shader.frag");


	// Set up vertex data (and buffer(s)) and attribute pointers使用纹理坐标更新顶点数据
	GLfloat vertices[] = {
	...
	// TexCoord attribute
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), (GLvoid*)(6 * sizeof(GLfloat))); // 顶点的纹理属性指针
	...
	int width, height;
	unsigned char* image = SOIL_load_image("container.jpg", &width, &height, 0, SOIL_LOAD_RGB);  // 加载图片
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1i(glGetUniformLocation(ourShader.Program, "ourTexture1"), 0);  //设置uniform采样器的位置值，或者说纹理单元
